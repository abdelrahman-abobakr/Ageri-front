...
const DashboardPage = () => {
  ...
  // Mock statistics for admin/moderator - in real app, these would come from API
  const stats = {
    totalUsers: 156,
    ...
    switch (userRole) {
      case USER_ROLES.ADMIN:
        return [
          ...
          { title: 'Pending Requests', value: stats.pendingRequests, icon: <ToolOutlined />, color: '#faad14' },
          { title: 'Active Courses', value: stats.activeCourses, icon: <ReadOutlined />, color: '#722ed1' },
        ];
      case USER_ROLES.MODERATOR:
        return [
          { title: 'Publications to Review', value: 15, icon: <BookOutlined />, color: '#1890ff' },
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        {user?.role === USER_ROLES.RESEARCHER && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <Title level={4} style={{ margin: 0, color: '#1890ff' }}>My Statistics</Title>
            <Button
              icon={<ReloadOutlined />}
        ...
        <Row gutter={[16, 16]}>
          {roleStats.map((stat, index) => (
            <Col xs={24} sm={12} md={6} key={index}>
              <Card>
                {statsLoading && user?.role === USER_ROLES.RESEARCHER ? (
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={12}>
          <Card title="Announcements" extra={<Button type="link">View All</Button>}>
            <div style={{ padding: '16px 0' }}>
              <Paragraph>
                <strong>System Maintenance:</strong> Scheduled maintenance on Sunday, 2:00 AM - 4:00 AM
              </Paragraph>
              <Paragraph>
                <strong>New Feature:</strong> Enhanced publication submission workflow now available
              </Paragraph>
              <Paragraph>
                <strong>Training:</strong> New course "Research Ethics" starting next week
              </Paragraph>
            </div>
          </Card>
        </Col>
